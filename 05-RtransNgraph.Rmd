...
# Apply and basic graphics {#rapplyandgrapics}

* https://youtu.be/IOMFhb37PTc
...
iris_list <- split(iris, iris$Species)
iris2 <- lapply(iris_list, function(x){subset(x, select=-Species)})
mydf <- lapply(iris2, function(x){apply(x, 2, mean)})
mydf2 <- rbind(mydf$setosa, mydf$versicolor, mydf$virginica)
barplot(mydf2, beside = T, legend.text = c("setosa", "versicolor", "virginica"))

#lapply(iris2, function(x){apply(x, 2, sd)})
